...
function get_azure_environment () {
  metadata_response=$(curl "http://169.254.169.254/metadata/instance/compute?api-version=2018-02-01" -H "Metadata:true")
  location=$(echo $metadata_response | jq .location -r)

  endpoints_response=$(curl "https://management.azure.com/metadata/endpoints?api-version=2017-12-01")
